...
bool ImGui::HotkeyButton(const char* label, const ImVec2& size_arg)
{
    // TODO: make this shit
    ImGuiWindow* window = GetCurrentWindow();
    if (window->SkipItems)
...
bool ImGui::Checkbox(const char* label, bool* v)
{
    ...
    else if (*v)
    {
        const float pad = ImMax(1.0f, IM_FLOOR(square_sz / 6.0f));
        //RenderCheckMark(window->DrawList, check_bb.Min + ImVec2(pad, pad), check_col, square_sz - pad * 2.0f);
        window->DrawList->AddRectFilled(check_bb.Min + ImVec2(2.f, 2.f), check_bb.Max - ImVec2(2.f, 2.f), check_col, 3.5f);
    }
...
bool ImGui::Checkbox(const char* label, bool* v)
{
    ...
    if (*v) {
        ...
        //reset canvas
        //Color of the enabled checkbox
        window->DrawList->AddRectFilled(ImVec2(newRect.Min.x - 1.f, newRect.Min.y - 1.f), ImVec2(newRect.Max.x + .5f, newRect.Max.y + 1.f), ImColor(58, 159, 255), 10);
        ...
        //Color of the disabled checkbox
...
bool ImGui::SliderScalar(const char* label, ImGuiDataType data_type, void* p_data, const void* p_min, const void* p_max, const char* format, ImGuiSliderFlags flags)
{
    ...
    const ImU32 frame_col = GetColorU32(g.ActiveId == id ? ImGuiCol_FrameBgActive : g.HoveredId == id ? ImGuiCol_FrameBgHovered : ImGuiCol_FrameBg);
    RenderNavHighlight(frame_bb, id);
    RenderFrame(frame_bb.Min, frame_bb.Max, frame_col, true, 10.f);
    ...
    {
        //window->DrawList->AddRectFilled(grab_bb.Min, grab_bb.Max, GetColorU32(g.ActiveId == id ? ImGuiCol_SliderGrabActive : ImGuiCol_SliderGrab), style.GrabRounding);
        window->DrawList->AddRectFilled(frame_bb.Min, ImVec2(grab_bb.Max.x + 2, frame_bb.Max.y), GetColorU32(g.ActiveId == id ? ImGuiCol_SliderGrabActive : ImGuiCol_SliderGrab), 10.f);
    }
...
bool ImGui::TreeNodeBehavior(ImGuiID id, ImGuiTreeNodeFlags flags, const char* label, const char* label_end)
{
    ...
    if (display_frame)
    {
        // Framed type
        // const ImU32 bg_col = GetColorU32((held && hovered) ? ImGuiCol_HeaderActive : hovered ? ImGuiCol_HeaderHovered : ImGuiCol_Header);
        const ImU32 bg_col = GetColorU32((held && hovered) ? style.Colors[ImGuiCol_HeaderActive] : hovered ? style.Colors[ImGuiCol_HeaderHovered] : ImVec4(0.f, 0.f, 0.f, 0.f));
        //RenderFrame(frame_bb.Min, frame_bb.Max, bg_col, true, style.FrameRounding);
        RenderNavHighlight(frame_bb, id, nav_highlight_flags);
        if (flags & ImGuiTreeNodeFlags_Bullet)
            RenderBullet(window->DrawList, ImVec2(text_pos.x - text_offset_x * 0.60f, text_pos.y + g.FontSize * 0.5f), text_col);
        ...
        {
            RenderFrame(ImVec2(text_pos.x - text_offset_x + padding.x - 3, text_pos.y - 3), ImVec2(text_pos.x - text_offset_x + padding.x + 16, text_pos.y + 16), bg_col, true, 10.f);
            RenderArrow(window->DrawList, ImVec2(text_pos.x - text_offset_x + padding.x, text_pos.y), text_col, is_open ? ImGuiDir_Down : ImGuiDir_Right, 1.0f);
        }
        else // Leaf without bullet, left-adjusted text
            text_pos.x -= text_offset_x;
